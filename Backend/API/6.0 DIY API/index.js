...
app.get("/filter", (req, res) => {
  ...
  const foundType = jokes.filter((joke) => joke.type === jokeType);
  res.json(foundType);
});
...
app.patch("/jokes/:id", (req, res) => {
  ...
  if (text !== undefined) jokes[searchIndex].jokeText = text;
  if (type !== undefined) jokes[searchIndex].jokeType = type;

  res.json({ message: "Joke updated successfully", joke: jokes[searchIndex] });
});
...
app.delete("/jokes/:id", (req, res) => {
  ...
  if (searchIndex > -1) {
    ...
    res.status(400).json({ message: "could not find joke" });
  }
});
...
app.delete("/jokes/all", (req, res) => {
  ...
  if (key === masterKey) {
    ...
    res
      .status(404)
      .json({ message: "could not preform task wrong password given" });
  }
});
